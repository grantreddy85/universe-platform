...
import {
  ...
  FileText, ShieldCheck, BookOpen, ExternalLink, Star, Image, FileSearch, Edit2, Save, X, Loader2, Shield
} from "lucide-react";
...
export default function AssetDetail() {
  ...
  const { data: linkedNote } = useQuery({
    queryKey: ["asset-note", asset?.note_id],
    queryFn: () => base44.entities.Note.filter({ id: asset.note_id }),
    enabled: !!asset?.note_id,
    select: (data) => data[0],
  });
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {!editingAttribution ? (
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" size="sm" className="h-7 text-xs text-emerald-600" onClick={() => { updateMutation.mutate({ attribution: pendingAttribution }); setEditingAttribution(false); logAuditEvent("attribution_updated", "Attribution breakdown updated.", { attribution: pendingAttribution }); }}>
                    <Save className="w-3 h-3 mr-1" /> Save
                  </Button>
