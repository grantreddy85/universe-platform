...
import {
  Plus, GitBranch, MoreHorizontal, Trash2, Play, CheckCircle2, XCircle,
  Clock, Sparkles, ChevronLeft, Save, Box
} from "lucide-react";
...
const statusIcons = { draft: Clock, running: Play, completed: CheckCircle2, failed: XCircle };
const statusStyles = {
...
export default function WorkflowsTab({ project }) {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.Workflow.create({ ...data, project_id: project.id }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["project-workflows", project.id] }),
  });

  const openEditor = (wf) => {
    setOpenWorkflow(wf);
    const saved = wf.parameters?.pipeline || { nodes: [], edges: [] };
    setPipeline(saved);
  };
  ...
  const savePipeline = async () => {
    ...
    setSaving(true);
    await base44.entities.Workflow.update(openWorkflow.id, {
      parameters: { ...openWorkflow.parameters, pipeline },
    });
    queryClient.invalidateQueries({ queryKey: ["project-workflows", project.id] });
    setSaving(false);
  };
  ...
  if (openWorkflow) {
    const StatusIcon = statusIcons[openWorkflow.status] || Clock;
    return (
      <div className="flex flex-col h-full">
        {/* Toolbar */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-white flex-shrink-0">
          <button
            onClick={() => setOpenWorkflow(null)}
          ...
          <span className="text-sm font-semibold text-gray-800">{openWorkflow.title}</span>
          <Badge variant="secondary" className={`text-[10px] uppercase ml-1 ${statusStyles[openWorkflow.status] || statusStyles.draft}`}>
            <StatusIcon className="w-2.5 h-2.5 mr-1" />{openWorkflow.status || "draft"}
          </Badge>
          <span className="text-[10px] text-gray-400 ml-auto">{pipeline.nodes.length} nodes · {pipeline.edges.length} edges</span>
          <Button
            size="sm"
        ...
        <div className="flex-1 min-h-0">
          <WorkflowCanvas
            ...
            onChange={(val) => setPipeline(typeof val === "function" ? val(pipeline) : val)}
          />
        </div>
      </div>
    );
      ...
      <div className="flex-1 p-6 lg:p-8 overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => setShowNew(true)} size="sm" className="bg-blue-600 hover:bg-blue-700 text-xs">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {workflows.map((wf) => {
              ...
              const nodeCount = wf.parameters?.pipeline?.nodes?.length || 0;
              const edgeCount = wf.parameters?.pipeline?.edges?.length || 0;
              return (
                <div
                  key={wf.id}
                  className="bg-white rounded-lg border border-gray-100 p-5 hover:border-indigo-200 transition-colors cursor-pointer"
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{wf.type?.replace(/_/g, " ") || "Unspecified"}</p>
                      {wf.description && <p className="text-xs text-gray-500 mt-2 line-clamp-1">{wf.description}</p>}
                      {nodeCount > 0 && (
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-[10px] text-indigo-500 font-medium bg-indigo-50 px-2 py-0.5 rounded-full">
                            {nodeCount} nodes · {edgeCount} edges
                          </span>
                          <span className="text-[10px] text-gray-400">Click to edit pipeline →</span>
                        </div>
                      )}
                      {nodeCount === 0 && (
                        <p className="text-[10px] text-gray-300 mt-2">No pipeline built yet — click to open canvas</p>
                      )}
                    </div>
                    <DropdownMenu>
        ...
        <Dialog open={showNew} onOpenChange={setShowNew}>
          <DialogContent className="sm:max-w-md">
            ...
            <form
              onSubmit={e => { e.preventDefault(); if (!form.title.trim()) return; createMutation.mutate(form); }}
              className="space-y-4 mt-2"
            >
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-gray-500">Title *</Label>
                <Input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. RNA-seq Differential Expression" className="text-sm" />
              </div>
              <div className="space-y-1.5">
              ...
              <DialogFooter className="pt-2">
                ...
                <Button type="submit" size="sm" className="bg-blue-600 hover:bg-blue-700 text-xs" disabled={!form.title.trim() || createMutation.isPending}>
